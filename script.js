...
const SHIPS = {
  VIPER:{id:'VIPER',name:'Viper Alpha',cost:0,speed:8,hp:100,shield:55,energy:100,damage:10,color:'#00f0ff',shape:'viper',special:'NOVA BURST',desc:'Balanced tactical interceptor.'},
  AEGIS:{id:'AEGIS',name:'Aegis Sentinel',cost:500,speed:6,hp:190,shield:120,energy:105,damage:13,color:'#7d4dff',shape:'aegis',special:'AEGIS WALL',desc:'Heavy armor and shield specialist.'},
  PHANTOM:{id:'PHANTOM',name:'Phantom Speeder',cost:900,speed:11,hp:78,shield:45,energy:120,damage:9,color:'#ff1768',shape:'phantom',special:'TIME SLICE',desc:'Extreme speed and rapid fire.'},
  SPECTRE:{id:'SPECTRE',name:'Spectre Interceptor',cost:1500,speed:9.5,hp:130,shield:80,energy:125,damage:15,color:'#00ffcc',shape:'spectre',special:'PHASE BEAM',desc:'Critical-hit energy craft.'},
  TITAN:{id:'TITAN',name:'Titan Dreadnought',cost:2600,speed:5,hp:330,shield:170,energy:105,damage:21,color:'#ffae00',shape:'titan',special:'GRAVITY HAMMER',desc:'Slow fortress with huge firepower.'},
  WRAITH:{id:'WRAITH',name:'Wraith Reaper',cost:3900,speed:10,hp:145,shield:95,energy:155,damage:18,color:'#ff4dff',shape:'wraith',special:'VOID DRAIN',desc:'Steals energy from enemy formations.'},
  SOLARIS:{id:'SOLARIS',name:'Solaris Ark',cost:5400,speed:7.5,hp:245,shield:135,energy:185,damage:19,color:'#ffe04d',shape:'solaris',special:'SOLAR FLARE',desc:'Energy carrier with radial weapons.'},
  OMEGA:{id:'OMEGA',name:'Omega Ascendant',cost:7800,speed:9,hp:270,shield:185,energy:210,damage:25,color:'#ffffff',shape:'omega',special:'OMEGA RIFT',desc:'Prototype with exceptional all-round stats.'},
  NOVA:{id:'NOVA',name:'Nova Valkyrie',cost:10500,speed:8.5,hp:220,shield:150,energy:240,damage:29,color:'#62a8ff',shape:'nova',special:'STARFALL',desc:'Late-game carrier with devastating energy output.'},
  QUASAR:{id:'QUASAR',name:'Quasar Monarch',cost:14000,speed:10,hp:310,shield:210,energy:260,damage:34,color:'#8affff',shape:'quasar',special:'QUASAR LANCE',desc:'Final-tier warship built for level 100.'}
};
...
class AudioFX{
  constructor(){this.ctx=null;this.master=.035}
  ...
  laser(enemy=false){this.tone(enemy?220:900,.08,enemy?'sawtooth':'sine',.25)}
...
class Bullet{
  constructor(x,y,vx,vy,color,enemy=false,damage=10,radius=3.5,homing=0){Object.assign(this,{x,y,vx,vy,color,isEnemy:enemy,damage,radius,homing,age:0})}
  ...
  draw(c){c.save();c.fillStyle=this.color;c.shadowBlur=12;c.shadowColor=this.color;c.beginPath();c.arc(this.x,this.y,this.radius,0,Math.PI*2);c.fill();c.restore()}
}
class PowerUp{
...
class Enemy{
  constructor(x,y,level,type='STANDARD'){this.x=x;this.y=y;this.type=type;this.size=type==='ELITE'?50:38;this.maxHp=(type==='ELITE'?90:28)+level*11;this.hp=this.maxHp;this.speed=(Math.random()*1.2+.8)+(level*.008);this.timer=30+Math.random()*80;this.color=type==='ELITE'?'#ffcc00':'#ff1768';this.phase=Math.random()*6}
  update(w){this.y+=this.speed;this.phase+=.04;this.x+=Math.sin(this.phase)*.5;if(this.y>w+this.size)this.y=-this.size;this.timer--}
  canShoot(){if(this.timer<=0&&this.y>0){this.timer=55+Math.random()*100;return true}return false}
  draw(c){c.save();c.translate(this.x,this.y);c.rotate(Math.sin(this.phase)*.08);c.fillStyle=this.color;c.strokeStyle=this.color;c.shadowBlur=14;c.shadowColor=this.color;c.beginPath();c.moveTo(0,this.size/2);c.lineTo(-this.size/2,-this.size/3);c.lineTo(0,-this.size/6);c.lineTo(this.size/2,-this.size/3);c.closePath();c.fill();c.stroke();if(this.type==='ELITE'){c.strokeStyle='#fff';c.beginPath();c.arc(0,0,this.size*.32,0,Math.PI*2);c.stroke()}c.restore()}
}
...
class Player{
  constructor(w,h,up,ship){this.config=ship;this.width=46;this.height=46;this.x=w/2;this.y=h-105;this.vx=0;this.vy=0;this.speed=ship.speed+up.speed*.32;this.maxHp=ship.hp+up.hull*13;this.hp=this.maxHp;this.maxShield=ship.shield+(up.shield-1)*30;this.shield=this.maxShield;this.maxEnergy=ship.energy+(up.energy-1)*9;this.energy=this.maxEnergy;this.damage=ship.damage*(1+up.damage*.11);this.fireRate=Math.max(2,10-(up.fireRate-1)*.22);this.crit=Math.min(.6,.03+up.crit*.025);this.magnet=65+up.magnet*14;this.fireCooldown=0;this.specialCooldown=0;this.active={}}
  update(keys,touch,w,h){if(touch){this.x+=(touch.x-this.x)*.18;this.y+=(touch.y-this.y)*.18}else{let dx=0,dy=0;if(keys.ArrowLeft||keys.a)dx--;if(keys.ArrowRight||keys.d)dx++;if(keys.ArrowUp||keys.w)dy--;if(keys.ArrowDown||keys.s)dy++;const m=Math.hypot(dx,dy)||1;this.vx=dx/m*this.speed;this.vy=dy/m*this.speed;if(!dx)this.vx*=.82;if(!dy)this.vy*=.82;this.x+=this.vx;this.y+=this.vy}this.x=Math.max(30,Math.min(w-30,this.x));this.y=Math.max(45,Math.min(h-50,this.y));if(this.fireCooldown>0)this.fireCooldown--;if(this.specialCooldown>0)this.specialCooldown--;this.energy=Math.min(this.maxEnergy,this.energy+.06);for(const k of Object.keys(this.active)){this.active[k]--;if(this.active[k]<=0)delete this.active[k]}}
  draw(c){
...
class GameEngine{
  constructor(){this.canvas=document.getElementById('gameCanvas');this.ctx=this.canvas.getContext('2d');this.audio=new AudioFX();this.state=GAME.MENU;this.keys={};this.touch=null;this.level=1;this.wave=1;this.wavesPerLevel=4;this.score=0;this.coins=Number(localStorage.getItem('nebula_coins')||0);this.bestScore=Number(localStorage.getItem('nebula_best_score')||0);this.bestStage=Number(localStorage.getItem('nebula_best_stage')||1);this.bossesDefeated=0;this.combo=1;this.bestCombo=1;this.comboTimer=0;this.upgrades=this.load('nebula_upgrades',{damage:1,fireRate:1,shield:1,hull:1,speed:1,crit:1,energy:1,magnet:1});this.unlockedShips=this.load('nebula_ships',['VIPER']);this.selectedShip=localStorage.getItem('nebula_selected_ship')||'VIPER';if(!this.unlockedShips.includes('VIPER'))this.unlockedShips.unshift('VIPER');this.player=null;this.enemies=[];this.bullets=[];this.powerUps=[];this.particles=[];this.texts=[];this.boss=null;this.transition=false;this.backgroundIndex=0;this.stars=[];this.resize();this.lastTime=0;this.accumulator=0;this.bind();this.renderShips();this.renderUpgrades();this.updateMenu();requestAnimationFrame(t=>this.loop(t))}
  ...
  resize(){
    const oldW=this.canvas.width||window.innerWidth,oldH=this.canvas.height||window.innerHeight;
    const w=Math.max(320,Math.floor(window.innerWidth||320));
    const h=Math.max(480,Math.floor(window.innerHeight||480));
    this.canvas.width=w;
    ...
    if(!this.stars.length){
      this.stars=Array.from({length:180},()=>({x:Math.random()*w,y:Math.random()*h,z:Math.random()*1.8+.2,s:Math.random()*1.6+.2}));
    }else{
      const sx=w/oldW,sy=h/oldH;
      for(const star of this.stars){
        star.x=Math.max(0,Math.min(w,star.x*sx));
        star.y=Math.max(0,Math.min(h,star.y*sy));
      }
    }
    if(this.player){
      this.player.x=Math.max(30,Math.min(w-30,this.player.x));
      this.player.y=Math.max(45,Math.min(h-50,this.player.y));
    }
  }
  bind(){window.addEventListener('resize',()=>this.resize());window.addEventListener('keydown',e=>{this.keys[e.key]=true;if([' ','ArrowUp','ArrowDown','ArrowLeft','ArrowRight'].includes(e.key))e.preventDefault();if(e.key==='p'||e.key==='P')this.togglePause();if(e.key==='e'||e.key==='E')this.useSpecial()});window.addEventListener('keyup',e=>this.keys[e.key]=false);const move=e=>{if(this.state===GAME.PLAYING&&e.touches[0])this.touch={x:e.touches[0].clientX,y:e.touches[0].clientY}};this.canvas.addEventListener('touchstart',move,{passive:true});this.canvas.addEventListener('touchmove',move,{passive:true});this.canvas.addEventListener('touchend',()=>this.touch=null);document.getElementById('startBtn').onclick=()=>{this.audio.init();this.startGame()};document.getElementById('restartBtn').onclick=()=>{this.audio.init();this.startGame()};document.getElementById('menuBtn').onclick=()=>this.toMenu();document.getElementById('upgradeBtn').onclick=()=>this.openHangar();document.getElementById('closeHangarBtn').onclick=()=>this.toMenu();document.getElementById('resumeBtn').onclick=()=>this.togglePause();document.getElementById('resetBtn').onclick=()=>{if(confirm('Reset all progress?')){localStorage.clear();location.reload()}};document.querySelectorAll('.tab').forEach(t=>t.onclick=()=>this.switchTab(t.dataset.tab))}
  ...
  toMenu(){this.state=GAME.MENU;document.getElementById('hud').classList.add('hidden');document.getElementById('gameOverMenu').classList.add('hidden');document.getElementById('pauseOverlay').classList.add('hidden');document.getElementById('upgradeMenu').classList.add('hidden');document.getElementById('mainMenu').classList.remove('hidden');this.updateMenu()}
  shipSVG(s){return `<svg viewBox="0 0 100 80" aria-hidden="true"><g fill="rgba(0,0,0,.75)" stroke="${s.color}" stroke-width="3" stroke-linejoin="round"><path d="${s.shape==='titan'||s.shape==='aegis'?'M50 4 L88 55 L70 72 L50 61 L30 72 L12 55 Z':s.shape==='phantom'||s.shape==='wraith'?'M50 3 L95 68 L50 54 L5 68 Z':s.shape==='solaris'||s.shape==='nova'?'M50 2 L66 30 L94 45 L65 49 L50 75 L35 49 L6 45 L34 30 Z':s.shape==='omega'||s.shape==='quasar'?'M50 2 L91 61 L63 52 L50 77 L37 52 L9 61 Z':'M50 2 L89 68 L50 53 L11 68 Z'}"/><path d="M50 17 L50 60 M31 55 L50 45 L69 55" stroke="#fff" stroke-width="2"/><circle cx="50" cy="42" r="5" fill="${s.color}" stroke="none"/></g></svg>`}
  renderShips(){const c=document.getElementById('shipCards');c.innerHTML='';Object.values(SHIPS).forEach(s=>{const unlocked=this.unlockedShips.includes(s.id),selected=this.selectedShip===s.id;const el=document.createElement('article');el.className=`ship-card ${selected?'selected':''}`;el.innerHTML=`<div class="ship-art" style="--ship:${s.color}">${this.shipSVG(s)}</div><h3 style="color:${s.color}">${s.name}</h3><p>${s.desc}</p><div class="ship-stats">SPD ${s.speed} · HP ${s.hp} · DMG ${s.damage}<br><strong>${s.special}</strong></div><button class="btn mini ${selected?'primary':''}">${unlocked?(selected?'EQUIPPED':'EQUIP'):`UNLOCK ${s.cost.toLocaleString()}`}</button>`;el.querySelector('button').onclick=()=>unlocked?this.selectShip(s.id):this.buyShip(s.id);c.appendChild(el)})}
  ...
  startGame(){this.audio.init();this.state=GAME.PLAYING;this.level=1;this.wave=1;this.score=0;this.combo=1;this.bestCombo=1;this.comboTimer=0;this.bossesDefeated=0;this.transition=false;this.backgroundIndex=0;this.player=new Player(this.canvas.width,this.canvas.height,this.upgrades,SHIPS[this.selectedShip]||SHIPS.VIPER);this.enemies=[];this.bullets=[];this.powerUps=[];this.particles=[];this.texts=[];this.boss=null;document.getElementById('mainMenu').classList.add('hidden');document.getElementById('upgradeMenu').classList.add('hidden');document.getElementById('gameOverMenu').classList.add('hidden');document.getElementById('pauseOverlay').classList.add('hidden');document.getElementById('hud').classList.remove('hidden');this.spawnLevel()}
  ...
  spawnWave(){if(this.state!==GAME.PLAYING||this.transition)return;this.enemies=[];if(this.level%5===0){this.boss=new Boss(this.canvas.width,this.level);document.getElementById('bossName').textContent=`${this.boss.def.name} // LV ${this.level}`;document.getElementById('bossPowerText').textContent=this.boss.def.power;document.getElementById('bossHpContainer').classList.remove('hidden');this.addText(this.canvas.width/2,this.canvas.height*.32,'BOSS DETECTED',this.boss.def.color,1.4);this.audio.boom(true);return}const count=Math.min(28,4+this.wave*2+Math.floor(this.level*.28));for(let i=0;i<count;i++){const elite=Math.random()<Math.min(.5,.12+this.level*.004);this.enemies.push(new Enemy(Math.random()*(this.canvas.width-80)+40,-60-Math.random()*550,this.level,elite?'ELITE':'STANDARD'))}}
  ...
  completeLevel(){if(this.level>=MAX_LEVEL){this.victory();return}this.transition=true;this.score+=this.level*500;this.coins+=40+this.level*8;this.level++;this.bestStage=Math.max(this.bestStage,this.level);this.save();this.addText(this.canvas.width/2,this.canvas.height*.5,`LEVEL ${this.level} UNLOCKED`,'#00ff9d',1.35);setTimeout(()=>{if(this.state===GAME.PLAYING){this.transition=false;this.spawnLevel()}},1400)}
  victory(){this.state=GAME.GAMEOVER;this.bestScore=Math.max(this.bestScore,this.score);this.bestStage=MAX_LEVEL;this.save();document.getElementById('gameOverTitle').textContent='FRONTIER CONQUERED';document.getElementById('gameOverReason').textContent='LEVEL 100 COMPLETE — THE HYPERION FRONTIER IS YOURS.';this.fillGameOver();document.getElementById('hud').classList.add('hidden');document.getElementById('gameOverMenu').classList.remove('hidden')}
  enemyBullet(x,y,vx,vy,color,dmg,radius=4,homing=0){this.bullets.push(new Bullet(x,y,vx,vy,color,true,dmg,radius,homing));this.audio.laser(true)}
  fire(){const p=this.player;if(!p||p.fireCooldown>0)return;let shots=2;if(p.active.QUAD)shots=4;else if(p.active.TRIPLE)shots=3;const mult=p.active.BOOST?2.4:1;const rapid=p.active.RAPID?Math.max(2,p.fireRate*.45):p.fireRate;p.fireCooldown=rapid;for(let i=0;i<shots;i++){const offset=shots===1?0:(i-(shots-1)/2)*13;const spread=shots>2?(i-(shots-1)/2)*.8:0;this.bullets.push(new Bullet(p.x+offset,p.y-22,spread,-14,p.config.color,false,p.damage*mult,3.7))}this.audio.laser(false)}
  ...
  applyDamage(amount){if(this.player.active.SHIELD)return;let d=amount;if(this.player.shield>0){const used=Math.min(this.player.shield,d);this.player.shield-=used;d-=used}if(d>0)this.player.hp-=d;if(this.player.hp<=0)this.gameOver()}
  explode(x,y,color,count=16,big=false){for(let i=0;i<count;i++)this.particles.push(new Particle(x,y,color,big?15:9,Math.random()*3+1,big?1.1:1));this.audio.boom(big)}
  ...
  update(){if(this.state!==GAME.PLAYING)return;const p=this.player;p.update(this.keys,this.touch,this.canvas.width,this.canvas.height);if((this.keys[' ']||this.touch)&&!this.transition)this.fire();if(this.keys['e']||this.keys['E']){this.keys['e']=false;this.keys['E']=false;this.useSpecial()}
    for(const b of this.bullets)b.update(p);this.bullets=this.bullets.filter(b=>b.x>-60&&b.x<this.canvas.width+60&&b.y>-80&&b.y<this.canvas.height+80);
  ...
  updateBoss(){this.boss.update(this.canvas.width,this)}
  updateEnemies(){
    const p=this.player;
    for(const e of this.enemies){
      e.update(this.canvas.width);
      if(e.canShoot()) this.enemyBullet(e.x,e.y+18,0,5.5,e.color,8+this.level*.15);
      if(Math.hypot(e.x-p.x,e.y-p.y)<34){
        this.explode(e.x,e.y,e.color,18);
        e.dead=true;
        this.applyDamage(25+this.level*.3);
      }
    }
  ...
  handleBulletHits(){
    const p=this.player;
    // Player shots hit enemies/bosses; enemy shots always hit the player.
    // Keeping both collision paths here prevents normal-wave shots from
    // silently passing through the ship (the old code only checked them in boss sectors).
    for(const b of this.bullets){
      if(b.dead) continue;
      if(b.isEnemy){
        if(Math.hypot(b.x-p.x,b.y-p.y)<Math.max(22,b.radius+18)){
          this.explode(b.x,b.y,'#ff1768',5);
          b.dead=true;
          this.applyDamage(b.damage);
        }
        continue;
      }
      if(this.boss){
        const d=Math.hypot(b.x-this.boss.x,b.y-this.boss.y);
        if(d<this.boss.size*.8){
          let dmg=b.damage;
          if(Math.random()<p.crit) dmg*=2.4;
          this.boss.hp-=dmg;
          b.dead=true;
          this.addText(b.x,b.y,`-${Math.round(dmg)}`,dmg>b.damage?'#fff36b':'#ffcc00',dmg>b.damage?1:.75);
      ...
      for(const e of this.enemies){
        ...
        if(Math.hypot(b.x-e.x,b.y-e.y)<e.size*.7){
          ...
          e.hp-=dmg;
          b.dead=true;
          this.addText(b.x,b.y,`-${Math.round(dmg)}`,dmg>b.damage?'#fff36b':'#fff',dmg>b.damage?1:.75);
  ...
  defeatBoss(){const b=this.boss;if(!b)return;this.explode(b.x,b.y,b.def.color,100,true);this.boss=null;this.bossesDefeated++;this.score+=Math.round((5000+this.level*900)*this.combo);this.coins+=200+this.level*12;this.combo=Math.min(20,this.combo+1.5);this.bestCombo=Math.max(this.bestCombo,this.combo);this.bullets=this.bullets.filter(x=>!x.isEnemy);document.getElementById('bossHpContainer').classList.add('hidden');this.addText(this.canvas.width/2,this.canvas.height*.5,`BOSS ${this.level} DESTROYED`,'#00ff9d',1.3);this.save();this.transition=true;setTimeout(()=>{if(this.state===GAME.PLAYING){this.transition=false;this.completeLevel()}},1500)}
  ...
  draw(){this.drawBackground();if(this.state===GAME.PLAYING||this.state===GAME.PAUSED){this.bullets.forEach(b=>b.draw(this.ctx));this.enemies.forEach(e=>e.draw(this.ctx));if(this.boss)this.boss.draw(this.ctx);this.powerUps.forEach(q=>q.draw(this.ctx));this.particles.forEach(x=>x.draw(this.ctx));this.texts.forEach(x=>x.draw(this.ctx));if(this.player)this.player.draw(this.ctx)}}
